...
@section('children')
    <div class="relative min-h-screen h-full flex justify-center flex-col items-end w-full image-translate bg-cover bg-left" 
        ...
        linear-gradient(120deg, transparent 50%, #fff 50%), 
        url('{{asset('images/lock-bg.jpg')}}');"
    >
        <img class="absolute left-5 top-5 w-[250px]" src="{{asset('images/logo-white.png')}}"/>
        <div class="w-[50%] pl-24 pt-16">
            <form method="POST" action="/api/login" class="flex justify-center flex-col gap-5 items-center">
                @csrf
                <div class="w-[400px]">
                    <p class="font-bold text-2xl border-b-4 pb-1 border-yellow-400 inline">Sign In</p>
                </div>
                ...
                <div class="flex flex-col gap-2 w-[400px] mt-5">
                    ...
                    <a href="/register" class="flex w-full cursor-pointer items-center justify-center overflow-hidden rounded-xl h-14 px-4 bg-skin-bg text-skin-text text-sm font-semibold leading-normal tracking-[0.015em]">
                        <span class="truncate">Don't have an account? Create Account</span>
                    </a>
                </div>
            </form>
        </div>
